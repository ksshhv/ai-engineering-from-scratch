...
- [Karpathy, "Neural Networks: Zero to Hero" — `makemore` series](https://github.com/karpathy/nn-zero-to-hero/tree/master/lectures/makemore) — the full bigram → MLP → RNN → WaveNet → Transformer walkthrough that makes the "drop recurrence" bet visceral. Start with `makemore_part1_bigrams.ipynb`, end with `makemore_part5_cnn1.ipynb`.
- [Raschka, *Build a Large Language Model (From Scratch)* — Ch. 2](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch02) — tokenization, embeddings, and the data pipeline that feeds a transformer. The `01_main-chapter-code` notebook gets you from raw text to input tensors in ~40 minutes.
